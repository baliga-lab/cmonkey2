...
RATIOS = dm.DataMatrix(NUM_GENES, NUM_COLS, GENE_NAMES, COL_NAMES)
RATIO_MATRICES = dm.DataMatrixCollection([RATIOS])
...
class CMonkeyTest(unittest.TestCase):  # pylint: disable-msg=R0904
    ...
    def test_create_cmonkey(self):
        ...
        cmonkey = cm.CMonkey(MockOrganism([]), RATIO_MATRICES)
    ...
    def test_run_cmonkey_with_networks(self):
        ...
        cmonkey = cm.CMonkey(MockOrganism([network1, network2]), RATIO_MATRICES)
        cmonkey.run()
        self.assertTrue(cmonkey.finished())
...
class MembershipTest(unittest.TestCase):  # pylint: disable-msg=R0904
    ...
    def test_map_to_is_member_matrix(self):
        ...
        in_matrix = [[1, 2], [2, 3]]
        out = cm.Membership.map_to_is_member_matrix(in_matrix, 3)
