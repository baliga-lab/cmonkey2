...
#TODO: non temp keep all files from meme and mast
#TODO: include known motifs in every xth search, score
...
class MotifScoringFunctionBase(scoring.ScoringFunctionBase):
    ...
    def __init__(self, organism, membership, matrix,
        ...
        self.__meme_dir = config_params['meme_dir']
    ...
    def compute_pvalues(self, iteration_result, iteration):
        ...
        These filters are applied in the order they appear in the list
         """
        logging.debug("Scoring in iteration number %s" %(iteration))
...
class ComputeScoreParams:
    ...
    def __init__(self, cluster, genes, seqs, used_seqs, meme_runner,
                 ...
                 iteration = 0):            # added iteration for naming of directories
        """constructor"""
...
def compute_cluster_score(params):
    ...
    if (nseqs >= params.min_cluster_rows
        ...
        run_result = params.meme_runner(params.seqs, params.used_seqs, params.iteration, params.cluster)
        pe_values = run_result.pe_values
        # run resutl is a meme.Meme.RunResult instance
